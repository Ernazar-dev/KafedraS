...
const SuperAdminSubjects = () => {
  ...
  const handleChange = (e) => {
    ...
    if (files) {
      ...
      if (name === "images")
        ...
        setPreviews({ ...previews, fileName: file.name });
      else if (name === "video")
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        ...
        showToast("Pán tabıslı jańalandı!");
      } else {
        await api.post("/subjects", data, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        showToast("Pán tabıslı qosıldı!");
      }
      fetchSubjects();
      resetForm();
    } catch (error) {
      showToast(error.response?.data?.message || "Qátelik júz berdi", "error");
    }
  };
  ...
  const handleEdit = (item) => {
    ...
    setPreviews({
      image: item.images ? getImageUrl(item.images) : null,
      fileName: item.file ? "Fayl bar" : null,
      videoName: item.video ? "Video bar" : null,
    });
  ...
  const handleDelete = async (id) => {
    try {
      ...
      showToast("Pán óshirildi");
    } catch (error) {
      showToast("Óshiriwdegi qátelik", "error");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f0f4ff] pb-20 sm:pb-0">
        ...
        <div className="flex-1 lg:ml-[72px]">
          ...
          <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                to="/"
                className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl border-2 border-slate-200 hover:border-red-300 hover:bg-red-50 text-slate-500 hover:text-red-500 text-[13px] font-semibold transition-all duration-200"
              >
                <LogOut size={14} />
                Shıǵıw
              </Link>
          ...
          {/* ─── BODY ─── */}
          <main className="px-3 sm:px-6 lg:px-8 py-4 sm:py-6 max-w-[1280px]">
            {/* ─── FORM CARD ─── */}
            <div
              className={`relative bg-white rounded-2xl border overflow-hidden mb-4 sm:mb-6 transition-all duration-300 ${
                editMode
            ...
            >
                ...
                <div className="flex items-center gap-2.5">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                      editMode ? "bg-amber-500" : "bg-[#02135e]"
                    }`}
                  >
                    {editMode ? (
                  ...
                  <span className="text-[14px] font-bold text-[#02135e]">
                    {editMode ? "Pándi redaktorlaw" : "Jańa pán qosıw"}
                  </span>
                </div>
                {editMode && (
                  <button
              ...
              <form onSubmit={handleSubmit} className="p-4 sm:p-6">
                <div className="flex flex-col gap-4">
                  {/* Cover image */}
                  <label className="relative w-full h-36 sm:h-48 rounded-xl overflow-hidden border-2 border-dashed border-slate-200 active:border-[#02135e] hover:border-[#02135e] transition-all duration-200 cursor-pointer group bg-slate-50">
                    {previews.image ? (
                      <>
                        <img
                          src={previews.image}
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <textarea
                      name="description"
                      placeholder="Pán haqqında maǵlıwmat..."
                    ...
                    <div className="flex flex-col sm:flex-row gap-2.5">
                      {/* PDF */}
                      <label
                        className={`flex items-center gap-2.5 px-4 py-3 sm:py-2.5 rounded-xl border-2 cursor-pointer transition-all duration-200 text-[14px] sm:text-[13px] font-semibold flex-1 active:scale-[0.98] ${
                          previews.fileName
                            ? "border-[#02135e] bg-[#02135e]/5 text-[#02135e]"
                            : "border-slate-200 bg-slate-50 text-slate-500 hover:border-[#02135e]/50"
                        }`}
                      >
                        <div
                    ...
                    <div className="flex flex-col sm:flex-row gap-2.5 mt-1">
                      <button
                        type="submit"
                        className={`w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3.5 sm:py-2.5 rounded-xl text-white font-bold text-[15px] sm:text-[13px] active:scale-95 transition-all duration-200 shadow-md ${
                          editMode
                            ? "bg-amber-500 hover:bg-amber-600 shadow-amber-500/25"
                            : "bg-[#02135e] hover:bg-[#02135e]/90 shadow-[#02135e]/25"
                        }`}
                      >
                        {editMode ? (
                          <CheckCircle size={17} />
                ...
                {subjects.map((item) => (
                  ...
                  >
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      <h3 className="text-[15px] font-extrabold text-[#02135e] mb-1.5 leading-snug line-clamp-1">
                        {item.name}
                      </h3>
              ...
              <div className="flex flex-col items-center justify-center py-20 gap-4 bg-white rounded-2xl border border-slate-200">
                ...
                <div className="text-center">
                  ...
                  <p className="text-slate-400 text-[13px]">
                    Joqarıdaǵı forma arqalı pán qosıń
                  </p>
                </div>
              </div>
            )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-30 sm:hidden bg-white/95 backdrop-blur-md border-t border-slate-200 flex">
        <Link
          to="/"
          className="flex-1 flex flex-col items-center justify-center gap-1 py-3 text-slate-400 active:text-red-500 transition-colors"
        >
          <LogOut size={20} />
          <span className="text-[10px] font-semibold">Shıǵıw</span>
        </Link>
        <button
          ...
          >
            ...
            <div className="px-6 py-7 flex flex-col items-center text-center">
              ...
              <p className="text-slate-400 text-[13px] leading-relaxed mb-6">
                Bul pán pútkilley óshiriledi.
                <br />
                Bul ámeldi qaytarıp bolmaydı.
              </p>
